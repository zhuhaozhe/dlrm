...
then
echo "--------------------------------------------"
echo "-------------test fp32 accuracy-------------"
echo "--------------------------------------------"
python dlrm_s_pytorch.py --inference-only --load-model=/lustre/dataset/dlrm/dlrm_py.pt --arch-sparse-feature-size=16 --arch-mlp-bot="13-512-256-64-16" --arch-mlp-top="512-256-1" --data-generation=dataset --data-set=kaggle --processed-data-file=/lustre/dataset/dlrm/kaggleAdDisplayChallenge_processed.npz --loss-function=bce --round-targets=True --learning-rate=0.1 --mini-batch-size=128 --print-freq=1024 --print-time 
    echo "--------------------------------------------"
elif [ $1 == throughput ]
then
echo "--------------------------------------------"
echo "------test fp32 performance throughput------"
echo "--------------------------------------------"
ncoros=4 #28 #12 #6
nsockets="0"
...
echo "--------------------------------------------"
echo "-------test fp32 performance realtime-------"
echo "--------------------------------------------"
ncoros=4 #28 #12 #6
nsockets="0"
dlrm_pt_bin="python dlrm_s_pytorch.py"
